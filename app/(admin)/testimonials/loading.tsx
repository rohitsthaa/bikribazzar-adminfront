function Sk({ className }: { className?: string }) {
  return <div className={`animate-pulse bg-stone-100 rounded-lg ${className ?? ''}`} />;
}

export default function TestimonialsLoading() {
  return (
    <main className="p-6 md:p-8 max-w-3xl">
      <div className="mb-6">
        <Sk className="h-8 w-36 rounded-xl mb-1" />
        <Sk className="h-4 w-48 rounded" />
      </div>

      {/* Add form */}
      <div className="bg-white rounded-2xl border border-stone-200 p-6 mb-5 space-y-4">
        <Sk className="h-4 w-28 rounded" />
        <div className="space-y-1.5">
          <Sk className="h-3.5 w-16 rounded" />
          <Sk className="h-20 w-full rounded-xl" />
        </div>
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Sk className="h-3.5 w-14 rounded" />
            <Sk className="h-10 w-full rounded-xl" />
          </div>
          <div className="space-y-1.5">
            <Sk className="h-3.5 w-20 rounded" />
            <Sk className="h-10 w-full rounded-xl" />
          </div>
        </div>
        <Sk className="h-9 w-28 rounded-xl" />
      </div>

      {/* List */}
      <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="px-5 py-4 border-b border-stone-100 space-y-2">
            <Sk className="h-4 w-full rounded" />
            <Sk className="h-4 w-3/4 rounded" />
            <Sk className="h-3.5 w-28 rounded mt-1" />
          </div>
        ))}
      </div>
    </main>
  );
}
